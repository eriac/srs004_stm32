#ifndef _ROS_s4_msgs_TrackedObjectArray_h
#define _ROS_s4_msgs_TrackedObjectArray_h
...
namespace s4_msgs
{
  ...
  class TrackedObjectArray : public ros::Msg
  {
    public:
      ...
      typedef s4_msgs::TrackedObject _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (s4_msgs::TrackedObject*)realloc(this->objects, objects_lengthT * sizeof(s4_msgs::TrackedObject));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(s4_msgs::TrackedObject));
      }
     return offset;
    }

    const char * getType(){ return "s4_msgs/TrackedObjectArray"; };
    const char * getMD5(){ return "800f2b1b898e3a6663c2a824ab442efe"; };
